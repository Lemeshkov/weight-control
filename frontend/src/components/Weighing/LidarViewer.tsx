...
const LidarViewer: React.FC = () => {
  ...
  const [vehicleParams, setVehicleParams] = useState({
    length_m: 6.0, // длина кузова (м)
    width_m: 2.5, // ширина кузова (м)
  ...
  const fetchLidarData = async () => {
    ...
    try {
      ...
      // Рассчитываем объем угля в кузове
  ...
  const calculateVolume = (data: LidarData) => {
    ...
    // 1. Находим уровень пустой дороги/платформы (максимальное расстояние)
    const roadLevel = Math.max(...data.distances_mm) / 1000;

    // 2. Вычисляем высоту угля в каждой точке
    const heights = data.distances_mm.map((d) => {
      const distM = d / 1000;
      // Если расстояние значительно меньше уровня дороги - есть уголь
      if (distM < roadLevel - 0.3) {
        return roadLevel - distM;
      }
      return 0;
    });

    // 3. Средняя высота угля по длине кузова
    const avgHeight = heights.reduce((a, b) => a + b, 0) / heights.length;

    // 4. Объем = длина × ширина × средняя высота
    const volume_m3 =
      vehicleParams.length_m * vehicleParams.width_m * avgHeight;

    // 5. Масса угля (тонны) = объем × плотность / 1000
    const mass_tons = (volume_m3 * vehicleParams.coal_density_kg_m3) / 1000;

    // 6. Площадь поперечного сечения (для информации)
    const crossSectionArea = avgHeight * vehicleParams.width_m;

    setVolumeData({
      volume_m3: Math.round(volume_m3 * 100) / 100,
      cross_section_area: Math.round(crossSectionArea * 100) / 100,
      avg_height_m: Math.round(avgHeight * 100) / 100,
  ...
  const drawLidarData = (data: LidarData) => {
    ...
    // Показываем расстояния до 10 метров
    const maxDisplayDistance = Math.min(maxDistance, 10);

    // Рисуем концентрические круги (1м, 2м, 3м, 4м)
    for (let i = 1; i <= 4; i++) {
    ...
    // Рисуем границы сектора 70° (от -35° до +35°)
    ...
    // Рисуем дугу, обозначающую сектор
    ctx.beginPath();
    ...
    // Рисуем линии через каждые 10 градусов внутри сектора
    ...
    // Рисуем точки сканирования (только в пределах сектора)
    // Угол между точками = общий угол сектора (70°) / количество точек
    const sectorAngleDeg = stopAngleDeg - startAngleDeg; // 70°
    ...
    for (let i = 0; i < data.distances_mm.length; i++) {
      ...
      // Текущий угол для этой точки (от -35° до +35°)
      ...
      if (distanceM < 1) {
        ctx.fillStyle = "#ff4444"; // Опасно близко
      } else if (distanceM < 3) {
        ctx.fillStyle = "#ffaa44"; // Внимание
      } else {
        ctx.fillStyle = "#44ff44"; // Нормально
      }
  ...
  //       ctx.fillStyle = "#ff4444"; // Опасно близко
  //     } else if (distanceM < 3) {
  //       ctx.fillStyle = "#ffaa44"; // Внимание (уголь близко к лидару)
  //     } else {
  //       ctx.fillStyle = "#44ff44"; // Нормально
  //     }
  ...
  const drawDistanceChart = (data: LidarData) => {
    ...
    // Порог 3 метра - уровень борта кузова
    ...
    >
          ...
          {lidarData &&
            ...
            lidarData.distances_m.length > 0 && (
              ...
              >
                ...
                >
                  🔴 Красная линия - уровень борта кузова (3 метра от лидара)
                </div>
                <div
                  style={{ fontSize: "11px", color: "#888", marginTop: "5px" }}
                >
                  📌 Выше красной линии - есть уголь, ниже - пустое место
                </div>
              </div>
            )}
        </div>
      ...
      <details style={{ marginTop: "20px" }}>
        <summary style={{ cursor: "pointer", color: "#666", fontSize: "12px" }}>
          ⚙️ Настройки автомобиля (для точного расчета объема)
        </summary>
        <div
          style={{
            marginTop: "10px",
...
//       <details style={{ marginTop: "20px" }}>
//         <summary style={{ cursor: "pointer", color: "#666", fontSize: "12px" }}>
//            Настройки автомобиля (для точного расчета объема)
//         </summary>
//         <div
//           style={{
//             marginTop: "10px",
