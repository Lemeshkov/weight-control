...
function App() {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:8000/api/weighing/current')
  ...
  const startWeighing = async () => {
    ...
    try {
      ...
      const response = await axios.get('http://localhost:8000/api/weighing/current');
  ...
  return (
    <div style={{ padding: 20, fontFamily: 'Arial, sans-serif' }}>
      <h1>⛏️ Weight Control System - Уголь-Контроль</h1>
      ...
      {activeTab === 'weighing' && (
        <>
          <div style={{ background: '#f0f0f0', padding: 15, borderRadius: 8, marginBottom: 20 }}>
            ...
            <p><strong>Статус:</strong> {health?.status === 'ok' ? '✅ Работает' : health?.status || '⏳ Проверка...'}</p>
            <p><strong>Текущий вес:</strong> <span style={{ fontSize: 20, fontWeight: 'bold' }}>{weight}</span> кг</p>
            <button 
