...
#             self._send_command("sMN SetAccessMode 3 F4724744")
#             time.sleep(0.2)
#             self._send_command("sMN Run")
#             time.sleep(0.2)
...
#     def get_scan_data(self, retry_count: int = 3) -> Optional[str]:
#         """Получить данные сканирования с повторными попытками"""
#         if not self.sock or not self.is_connected:
#             logger.error("Нет активного соединения с лидаром")
#             return None
    
#         for attempt in range(retry_count):
#             try:
#                 response = self._send_command("sRN LMDscandata")
            
#                 if response and "sRA LMDscandata" in response:
#                     logger.info(f"✅ Получены данные сканирования (попытка {attempt + 1})")
#                     return response
#                 else:
#                     logger.warning(f"Попытка {attempt + 1} не удалась, ответ: {response[:50] if response else 'None'}")
                
#             except socket.timeout:
#                 logger.warning(f"Таймаут при попытке {attempt + 1}")
...
#         logger.error(f"❌ Не удалось получить данные после {retry_count} попыток")
#         return None
...
#             for i, part in enumerate(parts):
#                 if part == "DIST1" and i + 1 < len(parts):
#                     # Парсим все значения после DIST1
#                     j = i + 1
#                     while j < len(parts) and parts[j] not in ["RSSI1", "RSSI2", "DIST2", "DEVICE"]:
#                         # Пробуем интерпретировать как расстояние
#                         try:
#                             hex_val = parts[j].strip()
#                             if hex_val:
#                                 value = int(hex_val, 16)
#                                 # Корректируем знак для 32-битных
#                                 if value > 0x7FFFFFFF:
#                                     value = value - 0x100000000
#                                 # Фильтруем разумные значения
#                                 if 0 <= value <= 50000:  # до 50 метров
#                                     result["distances_mm"].append(value)
#                         except ValueError:
#                             pass
#                         j += 1
...
#             # Конвертируем в метры и считаем статистику
#             if result["distances_mm"]:
#                 result["distances_m"] = [round(d/1000, 2) for d in result["distances_mm"]]
#                 result["points_count"] = len(result["distances_mm"])
...
#         if self.sock:
#             try:
#                 self._send_command("sMN StopMeas")
#                 self.sock.close()
#             except:
#                 pass
#             self.is_connected = False
